...
export default function CustomerPortal({ defaultTab = "profile" }: { defaultTab?: string }) {
  ...
  const downloadHTMLBill = (invoice: Purchase) => {
    const sub = parseFloat((invoice.totalAmount / 1.18).toFixed(2));
    const gst = parseFloat((invoice.totalAmount - sub).toFixed(2));
...
<head>
  ...
  <title>AR Supermarket Bill - ${invoice.id}</title>
  <style>
  ...
  <div class="receipt-card">
    ...
    <div class="meta-grid">
      <div class="meta-col">
        ...
        <div class="meta-value meta-val-bold">${invoice.customerName}</div>
        <div class="meta-value">Email: ${invoice.customerEmail}</div>
        <div class="meta-value">Mobile: +91 ${invoice.customerMobile}</div>
      </div>
      <div class="meta-col right">
        <div class="meta-title">Invoice Reference:</div>
        <div class="meta-value">Invoice ID: <span class="meta-val-bold" style="font-family: 'JetBrains Mono', monospace;">#${invoice.id}</span></div>
        <div class="meta-value">Date: ${invoice.purchaseDate}</div>
      </div>
    </div>
    ...
    <div class="summary-section">
      <table class="summary-table">
        <tr>
          <td>Subtotal before tax:</td>
          <td class="amount-val">₹${sub.toLocaleString("en-IN")}</td>
        </tr>
        <tr>
          <td>integrated GST (18%):</td>
          <td class="amount-val">₹${gst.toLocaleString("en-IN")}</td>
        </tr>
        <tr class="total-row">
          <td>GRAND TOTAL RECEIVED:</td>
          <td class="total-val text-right">₹${invoice.totalAmount.toLocaleString("en-IN")}</td>
        </tr>
      </table>
    </div>
    ...
    dlAnchorElem.setAttribute("download", `supermarket_bill_${invoice.id}.html`);
    dlAnchorElem.click();
    URL.revokeObjectURL(url);
    addToast("Supermarket Bill document downloaded successfully. Double click to view!", "success");
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 dark:bg-black/75 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-250 dark:border-slate-800 max-w-lg w-full rounded-3xl p-6 relative shadow-2xl animate-scale-up space-y-6 text-xs">
            ...
            <div className="flex gap-2.5 pt-2">
              <button
                onClick={() => downloadHTMLBill(recentInvoice)}
                className="flex-1 py-3 border border-emerald-200 dark:border-emerald-820 hover:bg-emerald-50/50 dark:hover:bg-slate-800 rounded-xl text-[11px] font-bold text-emerald-700 dark:text-emerald-400 flex items-center justify-center gap-1.5 transition"
              >
                <Download className="h-4 w-4" /> Download Bill (HTML)
              </button>
              <button
                onClick={() => triggerPrintReceipt(recentInvoice)}
                className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[11px] font-bold flex items-center justify-center gap-1.5 shadow-sm transition"
              >
      ...
      {viewHistoryRecord && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 dark:bg-black/75 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-250 dark:border-slate-800 max-w-lg w-full rounded-3xl p-6 relative shadow-2xl animate-scale-up space-y-6 text-xs animate-slide-in">
            ...
            <div className="flex gap-2.5 pt-2">
              <button
                onClick={() => downloadHTMLBill(viewHistoryRecord)}
                className="flex-1 py-3 border border-emerald-200 dark:border-emerald-820 hover:bg-emerald-50/50 dark:hover:bg-slate-800 rounded-xl text-[11px] font-bold text-emerald-700 dark:text-emerald-400 flex items-center justify-center gap-1.5 transition"
              >
                <Download className="h-4 w-4" /> Download Bill (HTML)
              </button>
